...
<xsl:when test="substring(upper-case($dbProductName),1,3)">select current timestamp from sysibm.sysdummy1</xsl:when>
...
<xsl:when test="substring(upper-case($dbProductName),1,3)">DB2Dialect</xsl:when>
