...
<xsl:value-of select="$charLt"/>%  //取出要修改的那条记录，并且回写表单
	if(isModify) {  //如果本页面是修改页面
		...
		out.print(RmVoHelper.writeBackMapToForm(RmVoHelper.getMapFromRequest(request, <xsl:value-of select="$ITableNameConstants"/>.DEFAULT_CONDITION_KEY_ARRAY)));  //输出表单回写方法的脚本
  	}
%>
