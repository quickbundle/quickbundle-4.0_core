...
<xsl:value-of select="$charLt"/>%  //取出List
    ...
    List<xsl:value-of select="$charLt"/>RmCommonVo> lvo = RmProjectHelper.getCommonServiceInstance().doQueryPage("select a.*, b.<xsl:value-of select="/meta/tables/table[@tableName=../@tableName]/@keyColumn"/> as RM_DISPLAY_COLUMN from <xsl:value-of select="@tableName"/> a join <xsl:value-of select="../@tableName"/> b on a.<xsl:value-of select="@refColumn"/> = b.<xsl:value-of select="../@refColumn"/> where a.<xsl:value-of select="@mainColumn"/>=" + <xsl:value-of select="lower-case(@mainColumn)"/>);
...
<xsl:value-of select="$charLt"/>script type="text/javascript">
    ...
    function add_onClick() {  //到增加记录页面
        ...
        getReference(new Array(form.<xsl:value-of select="lower-case(@refColumn)"/>s, inputValueName), '<xsl:value-of select="$charLt"/>%=request.getContextPath()%>/', '<xsl:value-of select="$charLt"/>%=request.getContextPath()%>/<xsl:value-of select="../@tableName"/>/reference?referenceInputType=checkbox');
...
<xsl:value-of select="$charLt"/>table class="tableHeader">
  <xsl:value-of select="$charLt"/>tr>
    ...
    <xsl:value-of select="$charLt"/>td class="tableHeaderMiddleTd"><xsl:value-of select="$charLt"/>b>关联的<xsl:value-of select="/meta/tables/table[@tableName=../@tableName]/@tableNameDisplay"/>列表<xsl:value-of select="$charLt"/>/b><xsl:value-of select="$charLt"/>/td>
    <xsl:value-of select="$charLt"/>td class="tableHeaderMiddleTd" width="60%" align="right">
		<xsl:value-of select="$charLt"/>input type="button" class="button_ellipse" id="button_add" value="新增" onclick="javascript:add_onClick();" title="新增关联的<xsl:value-of select="/meta/tables/table[@tableName=../@tableName]/@tableNameDisplay"/>"/>
...
<xsl:value-of select="$charLt"/>layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="rmOrderNumber" align="center" sortAction="0">
    ...
    <xsl:value-of select="$charLt"/>layout:collectionItem width="8%" title="名称">
		...
		<xsl:value-of select="$charLt"/>%="<xsl:value-of select="$charLt"/>a class='aul' target='_blank' href='" + request.getContextPath() + "/<xsl:value-of select="/meta/tables/table[@tableName=../@tableName]/@tableDirName"/>detail/" + <xsl:value-of select="lower-case(@refColumn)"/> + IGlobalConstants.REQUEST_IS_READ_ONLY +  "=1'>"%>
