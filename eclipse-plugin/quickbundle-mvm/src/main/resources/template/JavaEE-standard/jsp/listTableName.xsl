...
<xsl:value-of select="$charLt"/>%  //判断是否只读
	...
	if("1".equals(request.getParameter(<xsl:value-of select="$ITableNameConstants"/>.REQUEST_IS_READ_ONLY)) || "1".equals(request.getAttribute(<xsl:value-of select="$ITableNameConstants"/>.REQUEST_IS_READ_ONLY))) {
		isReadOnly = true;
	}
%>
</xsl:if>
...
<xsl:value-of select="$charLt"/>script type="text/javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action="<xsl:value-of select="$charLt"/>%=request.getContextPath()%>/" + rmActionName + ".do?cmd=find<xsl:value-of select="$charAmp"/>id=" + ids;
		form.submit();
	}
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
    	...
    	form.action="<xsl:value-of select="$charLt"/>%=request.getContextPath()%>/" + rmActionName + ".do?cmd=deleteMulti<xsl:value-of select="$charAmp"/>ids=" + ids;
    	form.submit();
	}
	function simpleQuery_onClick(){  //简单的模糊查询
    	form.action="<xsl:value-of select="$charLt"/>%=request.getContextPath()%>/" + rmActionName + ".do?cmd=simpleQuery";
    	form.submit();
  	}
  	<xsl:if test="contains(@customBundleCode, 'importExport')">
	...
	function detail_onClick(thisId) {  //实现转到详细页面
		...
		form.action="<xsl:value-of select="$charLt"/>%=request.getContextPath()%>/" + rmActionName + ".do?cmd=detail";
		form.submit();
	}
...
<xsl:value-of select="$charLt"/>div id="div_advanced" style="display:none;">
	<xsl:value-of select="$charLt"/>table class="table_query">
		...
		<xsl:value-of select="$charLt"/>/tr>
		<xsl:value-of select="$charLt"/>%-- 将需要隐藏的查询条件剪切到这里 --%>
	<xsl:value-of select="$charLt"/>/table>
<xsl:value-of select="$charLt"/>/div>
...
<xsl:value-of select="$charLt"/>table class="tableHeader">
  <xsl:value-of select="$charLt"/>tr>
    ...
    <xsl:value-of select="$charLt"/>td class="tableHeaderMiddleTd" width="60%" align="right">
		...
		<!--xsl:value-of select="$charLt"/>input type="button" class="button_ellipse" id="button_approve" value="审批" onclick="" title="审批单据"/--><xsl:if test="contains(@customBundleCode, 'importExport')">
...
<xsl:if test="contains(@customBundleCode, 'readonly')"><xsl:value-of select="$charLt"/>%=isReadOnly ? "<xsl:value-of select="$charLt"/>input type=\"hidden\" name=\"" + <xsl:value-of select="$ITableNameConstants"/>.REQUEST_IS_READ_ONLY + "\" value=\"1\">" : ""%></xsl:if>
