...
<xsl:value-of select="$charLt"/>script type="text/javascript">
	...
	function exportExcel_onClick(){  //导出到Excel
		...
		form.target = "_blank";
	    form.action="<xsl:value-of select="$charLt"/>%=request.getContextPath()%>/" + rmActionName + ".do?cmd=statistic_exportExcel";
    	form.submit();
	}  
...
<xsl:value-of select="$charLt"/>table class="table_div_content">
	<xsl:value-of select="$charLt"/>tr>
		<xsl:value-of select="$charLt"/>td align="right">
			...
			<xsl:value-of select="$charNbsp"/><xsl:value-of select="$charNbsp"/><xsl:value-of select="$charNbsp"/><xsl:value-of select="$charNbsp"/><xsl:value-of select="$charNbsp"/><xsl:value-of select="$charNbsp"/>
