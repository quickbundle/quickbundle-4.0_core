...
public class RmIdGenerator implements IRmIdGenerator {
    ...
    boolean isTrue(String multi_db) {
    	return "true".equals(multi_db) || "1".equals(multi_db);
    }
    ...
    void doInitIdBatch(Map<String, Element> mTableName_Ele) {
        ...
        for (Map.Entry<String, Element> en: mTableName_Ele.entrySet()) {
        	...
        	if(isTrue(eleTable.valueOf("@multi_db")) || batchSize >= MAX_BATCH_SIZE) {
        	...
        	batchSize ++;
        	if(isTrue(eleTable.valueOf("@multi_db"))){
        		doInitIdBatchQuery(todoTable);
        		batchSize = 0;
        		todoTable.clear();
        	}
    ...
    void doInitIdBatchQuery(Map<String, Element> todoTable) {
    	...
    	if(!RmBaseConfig.getSingleton().isGenerateIdFromDb()) {
    		try {
    			...
    			if(isTrue(todoTable.values().toArray(new Element[0])[0].valueOf("@multi_db"))) {
    ...
    void doBadSqlGrammar(String sql, String tableName, String clusterTablePrefix) {
        ...
        try {
            ...
            List<Node> nBeans = doc.selectNodes("/beans/node()[@parent='abstractDataSource']");
            for (Node nBean : nBeans) {
